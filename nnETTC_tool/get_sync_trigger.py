    ...
    with rosbag.Bag(rosbag_path, 'r') as bag:
        ...
        for exp_cnt, (_, exp_msg, _) in enumerate(bag.read_messages(topics=[exp_topic])):
            ...
            if not bool_first_frame and abs(dt - expect_time_interval) > expect_time_interval * 0.2:
                error_message = f"Error: {exp_cnt:010d}, RGB frequency is not [{int(rgb_expect_freq)}], the time difference is [{dt}]"
...
def match_triggers(rgb_trigger_df: pd.DataFrame, 
    ...
    for _, e_trigger_row in tqdm(event_trigger_df.iterrows(), total=len(event_trigger_df), desc="Matching triggers"):
        ...
        # Find the corresponding row in rgb_trigger_df
        threshold_ts = threshold_tms / 1000
        matching_row = rgb_trigger_df[(rgb_trigger_df['rgb_exp_start_ts'] - e_msg_header_ts).abs() < threshold_ts]
        
        if not matching_row.empty and len(matching_row) == 1:
            rgb_trigger_df.loc[matching_row.index, 'e_msg_header_ts'] = e_msg_header_ts   # second
            rgb_trigger_df.loc[matching_row.index, 'event_trigger_tnsec'] = e_trigger_row['event_trigger_tnsec']  # nanosecond
        else:
